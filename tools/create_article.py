...
def build_page_content(
    ...
    """Generate the inner article HTML fragment to be injected into the article template."""
    ...
    # Comments API left for global config; template includes script which uses data-root
    content = f'''
    <main class="article-main">
        <article class="article">
            <h1 class="article-title">{title}</h1>
            <p class="article-meta">{summary}</p>
            ...
            {ending}
            <section id="related" class="related"><h2>関連記事</h2><div id="relatedList"></div></section>
            <section id="comments" class="comments"><h2>コメント</h2><div id="comments-list"></div>
                <form id="commentForm" class="comment-form">
                    <input id="comment-name" placeholder="名前" />
                    <textarea id="comment-text" placeholder="コメントを入力"></textarea>
                    <div class="comment-actions"><button type="submit">投稿</button></div>
                </form>
            </section>
        </article>
    </main>
...
def create_article(project_root: Path, article: dict[str, Any]) -> Path:
    ...
    # Load template
    template_path = project_root / "templates" / "article_template.html"
    template = template_path.read_text(encoding="utf-8") if template_path.exists() else None

    for index, page in enumerate(pages):
        if template:
            content = build_page_content(article, page, index, pages, file_names)
            title = escape(article["title"])
            summary = escape(article["summary"])
            comments_api = os.environ.get('COMMENTS_API', '').strip()
            comments_attr = f' data-comments-api="{comments_api}"' if comments_api else ''
            page_html = template.replace("{{TITLE}}", title).replace("{{DATE}}", datetime.now().strftime("%Y-%m-%d")).replace("{{CONTENT}}", content).replace("{{RELATED_JSON}}", json.dumps([]))
            # Insert data-root and page index attributes
            page_html = page_html.replace("<body", f"<body data-root=\"../../\" data-page-index=\"{index}\"{comments_attr}")
            (article_directory / file_names[index]).write_text(page_html, encoding="utf-8")
        else:
            html = build_page_content(article, page, index, pages, file_names)
