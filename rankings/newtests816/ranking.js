...
let loadedComments = [];
let visibleCommentCount = COMMENTS_PER_PAGE;
...
function createCommentItem(comment, commentNumber) {
  return `
    <article class="commentItem">
      <header>
        <span class="commentNumber">${commentNumber}件目</span>
...
function renderComments() {
  ...
  const totalComments = loadedComments.length;
  const newestComments = [...loadedComments].reverse();
  const visibleComments = newestComments.slice(0, visibleCommentCount);

  container.innerHTML = visibleComments
    .map((comment, index) =>
      createCommentItem(comment, totalComments - index),
    )
    .join("");

  loadMoreButton.hidden = visibleCommentCount >= totalComments;
}
...
async function loadComments() {
  ...
  try {
    ...
    visibleCommentCount = COMMENTS_PER_PAGE;
    renderComments();
  } catch (error) {
    console.error(error);
...
document.querySelector("#loadMoreComments")?.addEventListener("click", loadMoreComments);
loadRelatedPages();
loadComments();
